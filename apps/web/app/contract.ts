...
export type ContractReplayRecord = {
  ...
  plies: number;
  moves: ContractMove[];
  initialPosition?: PositionContract;
...
export function validateContractReplay(value: unknown): ContractReplayRecord {
  ...
  const config = validateGameConfig(value.config);
  const initialPosition = value.initialPosition === undefined ? undefined : validatePosition(value.initialPosition);
  if (initialPosition) {
    if (initialPosition.config.boardSize !== config.boardSize || initialPosition.config.reservePerPlayer !== config.reservePerPlayer || initialPosition.config.maxPlies !== config.maxPlies) throw new Error("Initial position configuration does not match replay");
    if (initialPosition.winner !== null) throw new Error("Initial position must be non-terminal");
  }
  const provenance = value.provenance === undefined ? undefined : record(value.provenance, "replay provenance");
  ...
  return { contractVersion: 1, seed: integerInRange(value.seed, 0, 4_294_967_295) ? Number(value.seed) : (() => { throw new Error("Invalid replay seed"); })(), config, engine, agents, agentSpecifications, winner, result: value.result as "win" | "draw", reason: value.reason, plies, moves, ...(initialPosition ? { initialPosition } : {}), ...(provenance ? { provenance } : {}) };
}
