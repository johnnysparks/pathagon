import { applyAction, createGame, createGameFromPosition, legalActions } from "./pathagon.ts";
...
import type { AgentSpecification, ContractReplayRecord, EngineMetadata, GameConfig, PositionContract } from "./contract.ts";
...
export function validateSelfPlayRecord(value: unknown): SelfPlayGameRecord {
  ...
  const initialPosition = input.initialPosition === undefined ? undefined : validatePosition(input.initialPosition);
  if (initialPosition && (initialPosition.config.boardSize !== config.boardSize || initialPosition.config.reservePerPlayer !== config.reservePerPlayer || initialPosition.config.maxPlies !== config.maxPlies)) throw new Error("Initial position configuration does not match replay");
  if (initialPosition && initialPosition.winner !== null) throw new Error("Initial position must be non-terminal");
  ...
  let state = initialPosition ? createGameFromPosition(initialPosition) : createGame(config);
...
function validateProvenance(value: unknown): Record<string, unknown> {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error("Invalid self-play provenance");
