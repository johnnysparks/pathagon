...
def main() -> None:
    ...
    examples = load_replay_source(Path(args.data), BoardConfig(7, 14))
    ...
    model = None
    if args.checkpoint:
        model = load_model(Path(args.checkpoint), choose_device(args.device), qadv=True)
        model.eval()
    report = {
        "data": args.data,
        ...
        "metrics": evaluate(examples, model),
    }
