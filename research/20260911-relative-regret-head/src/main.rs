...
fn relative_scores(scores: &[i32]) -> (Vec<f32>, f32, f32) {
    if scores.is_empty() {
    ...
    let mut deviations = sorted
        .iter()
        .map(|score| (*score as f32 - median).abs())
        .collect::<Vec<_>>();
    deviations.sort_by(f32::total_cmp);
    let mad = deviations[deviations.len() / 2];
    let scale = (1.4826 * mad).max(250.0);
    let values = scores
        .iter()
        .map(|score| ((*score as f32 - median) / scale).clamp(-1.0, 1.0))
        .collect::<Vec<_>>();
    (values, median, scale)
}
...
fn export_root(root: &RootCandidate, config: Config) -> Option<serde_json::Value> {
    ...
    let (teacher_values, teacher_value_center, teacher_value_scale) =
        relative_scores(&ranked.iter().map(|item| item.score).collect::<Vec<_>>());
    let soft_policy = softmax(&teacher_values, 0.35);
    let horizons = [
    ...
    Some(json!({
        ...
        "teacherValueCenter": teacher_value_center,
        "teacherValueScale": teacher_value_scale,
        "teacherValueMethod": "within-position-median-mad-v1",
