...
fn main() {
    ...
    let model_bytes = fs::read(&model_path)
        .unwrap_or_else(|error| panic!("read board policy/value model {model_path}: {error}"));
    ...
    let candidate = if args.contains_key("qadv") {
        let model = Arc::new(
            OnnxQAdvModel::from_bytes(&model_bytes)
                .unwrap_or_else(|error| panic!("load board QAdv model: {error}")),
        );
        Agent::board_qadv_sorter_with_deadline(
            "pathfinder-board-qadv-v1-research",
            config,
            model,
            deadline,
        )
        ...
        let model = Arc::new(
            OnnxGnnPolicyValueModel::from_bytes(&model_bytes)
                .unwrap_or_else(|error| panic!("load board policy/value model: {error}")),
        );
        Agent::board_policy_sorter_with_deadline(
            "pathfinder-board-policy-value-v1-research",
            config,
            model,
            deadline,
        )
