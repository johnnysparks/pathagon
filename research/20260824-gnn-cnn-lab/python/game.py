...
class GameState:
    ...
    def seeded(
        ...
        """Construct a fresh, non-terminal position for a seeded episode.

        A seeded root may be legal without being historically reachable. The
        inventory, overlap, terminal-path, forbidden-square, and moveability
        checks here are the shared boundary for curriculum data generation and
        replay loading.
        """

        full = (1 << config.cell_count) - 1
        if light < 0 or dark < 0 or light & dark:
            raise ValueError("seeded position has overlapping or negative pieces")
        if (light | dark | forbidden) & ~full or forbidden & (light | dark):
            raise ValueError("seeded position has an out-of-range or occupied forbidden square")
        if len(reserves) != 2 or any(reserve < 0 for reserve in reserves):
            raise ValueError("seeded position reserves must be non-negative")
        if any(count_bits(pieces) + reserves[index] != config.reserve_per_player for index, pieces in enumerate((light, dark))):
            raise ValueError("seeded position violates the per-player inventory invariant")
        if ply < 0 or ply > config.max_plies:
            raise ValueError("seeded position ply is outside the configured limit")
        for marker in last_relocated_to:
            if marker is not None and not 0 <= marker < config.cell_count:
                raise ValueError("seeded position relocation marker is outside the board")
        state = cls(config, light, dark, tuple(reserves), turn, forbidden, last_relocated_to, 0, None, None, ply)
        if has_winning_path(state, Player.LIGHT) or has_winning_path(state, Player.DARK):
            raise ValueError("seeded position must not contain an active winning path")
        if not state.legal_actions():
            raise ValueError("seeded position must have at least one legal action")
        return state
