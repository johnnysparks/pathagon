...
def run_match(
    ...
    state = initial_state or GameState.initial(config)
    if state.config != config:
        raise ValueError("seeded match state does not match the requested board configuration")
...
def generate_game(
    ...
    result = run_match(config, seed, choose_action, progress=progress, initial_state=initial_state)
...
def game_record(
    ...
    root = initial_state or (examples[0].state if examples else GameState.initial(final_state.config))
    if root.light or root.dark or root.reserves != (final_state.config.reserve_per_player, final_state.config.reserve_per_player):
        record["initialPosition"] = position_record(root)
    return record


def position_record(state: GameState) -> dict:
    """Serialize a complete rule-relevant state for a seeded replay root."""
    ...
    return {
        "contractVersion": 1,
        "config": game_config(state.config.size, state.config.reserve_per_player, state.config.max_plies),
        "board": board,
