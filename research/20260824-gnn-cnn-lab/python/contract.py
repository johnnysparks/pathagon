...
def validate_replay_record(value: Any) -> dict:
    ...
    if "initialPosition" in value:
        initial = validate_position(value.get("initialPosition"))
        initial_config = initial["config"]
        if initial_config["boardSize"] != config["boardSize"] or initial_config["reservePerPlayer"] != config["reservePerPlayer"] or initial_config["maxPlies"] != config["maxPlies"]:
            raise ValueError("initial position configuration does not match replay")
        if initial.get("winner") is not None:
            raise ValueError("initial position must be non-terminal")
    if "provenance" in value and not isinstance(value["provenance"], Mapping):
