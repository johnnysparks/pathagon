...
class PipelineTest(unittest.TestCase):
    ...
    def test_seeded_root_round_trips_from_declared_initial_position(self) -> None:
        config = BoardConfig(3, 6, 36)
        state = GameState.seeded(config, 1 << 6, 1 << 4, (5, 5), Player.LIGHT)
        actions = tuple(state.legal_actions())
        action = next(candidate for candidate in actions if candidate == Action.place(1))
        policy = tuple(1.0 if candidate == action else 0.0 for candidate in actions)
        example = SearchExample(state, actions, policy, action, 1.0)
        record = game_record([example], state.apply_legal(action), seed=19, initial_state=state)
        self.assertIn("initialPosition", record)
        self.assertEqual(record["initialPosition"]["board"][6], "light")
        with TemporaryDirectory() as directory:
            path = Path(directory) / "seeded.jsonl"
            path.write_text(json.dumps(record) + "\n", encoding="utf-8")
            examples = load_replay_examples(path)
        self.assertEqual(examples[0].state, state)
        self.assertEqual(examples[0].action, action)

    def test_seeded_root_rejects_invalid_inventory_and_terminal_paths(self) -> None:
        config = BoardConfig(3, 6, 36)
        with self.assertRaisesRegex(ValueError, "inventory"):
            GameState.seeded(config, 1 << 6, 0, (6, 6), Player.LIGHT)
        with self.assertRaisesRegex(ValueError, "winning path"):
            GameState.seeded(config, (1 << 6) | (1 << 3) | (1 << 0), 0, (3, 6), Player.LIGHT)
