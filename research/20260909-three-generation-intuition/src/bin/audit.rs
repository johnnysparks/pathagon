...
use pathagon_engine::endgame::EndgameRepetitionKey;
use std::env;
use std::fs::File;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (line_number, line) in BufReader::new(file).lines().enumerate() {
        ...
        let mut repetitions = HashMap::<EndgameRepetitionKey, u8>::new();
        let mut sequence = Vec::with_capacity(record.moves.len());
        for movement in &record.moves {
            if state.winner.is_some() || state.ply >= state.config.max_plies {
                return Err("move after terminal state".into());
            }
            let count = repetitions.entry(EndgameRepetitionKey::from(state)).or_default();
            *count += 1;
            if *count >= 3 { return Err("move after threefold repetition".into()); }
        ...
        let final_visits = repetitions.get(&EndgameRepetitionKey::from(state)).copied().unwrap_or(0) + 1;
        let valid_reason = match record.reason.as_str() {
            "path" => state.winner.is_some(),
            "max-plies" => state.winner.is_none() && state.ply == state.config.max_plies,
            "threefold-repetition" => state.winner.is_none() && state.ply < state.config.max_plies && final_visits >= 3,
            "no-legal-action" => state.winner.is_none() && state.legal_actions().is_empty(),
            _ => false,
        };
        if !valid_reason { return Err(format!("invalid termination reason on seed {}: {}",record.seed,record.reason).into()); }
