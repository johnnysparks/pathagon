#!/usr/bin/env python3
"""Train an action-conditioned board model from quiet regret/value targets."""
...
def prepared(row: dict) -> tuple[GameState, tuple[Action, ...], torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    outcomes = torch.tensor(row["continuationOutcomes"], dtype=torch.float32)
    ...
    return state, candidates, values, policy, outcomes
...
def train(
    ...
    for _step in range(steps):
        ...
        state, actions, teacher_values, soft_policy, outcomes = prepared_rows[row_index]
        ...
        expected_outcome = (soft_policy * outcomes).sum()
        value_loss = F.mse_loss(value, expected_outcome)
        loss = (
            policy_loss
            + q_loss
            + 0.5 * advantage_loss
...
def evaluate(model: PathagonGNN, rows: list[dict]) -> dict:
    ...
    for row in rows:
        state, actions, teacher_values, soft_policy, outcomes = prepared(row)
        ...
        value_abs += abs(float(value.cpu()) - float(row["expectedContinuationOutcome"]))
