...
fn select_roots(buckets: BTreeMap<String, Bucket>, max_roots: usize) -> Vec<RootCandidate> {
    let mut strata = BTreeMap::<(u8, u8), Vec<RootCandidate>>::new();
    for (_key, bucket) in buckets {
        for root in bucket.roots {
            strata
                .entry((root.phase.index(), root.state.turn.index() as u8))
                .or_default()
                .push(root);
        }
    }
...
fn rollout_outcome(state: GameState, root_player: Player, plies: u8, config: SearchConfig) -> i8 {
    let mut state = state;
    for _ in 0..plies {
        if let Some(winner) = state.winner {
            return if winner == root_player { 1 } else { -1 };
        }
    ...
    state
        .winner
        .map_or(0, |winner| if winner == root_player { 1 } else { -1 })
}
...
fn export_root(root: &RootCandidate, config: Config) -> Option<serde_json::Value> {
    ...
    let outcomes = ranked
        ...
        .collect::<Vec<_>>();
    let has_outcome_difference = outcomes.iter().any(|outcome| *outcome != outcomes[0]);
    let source_outcome =
        root.observation.winner.map_or(
            0_i8,
            |winner| if winner == root.state.turn { 1 } else { -1 },
        );
    let expected_outcome = soft_policy
        .iter()
        .zip(outcomes.iter())
        .map(|(probability, outcome)| *probability * f32::from(*outcome))
        .sum::<f32>();
    Some(json!({
        "schemaVersion": 1,
        ...
        "partition": if stable_hash(&root.game_key) % 5 == 0 { "heldout" } else { "train" },
        ...
        "continuationOutcomes": outcomes,
        "expectedContinuationOutcome": expected_outcome,
...
fn main() -> Result<(), String> {
    ...
    let roots = select_roots(buckets, config.max_roots.saturating_mul(8));
    output
        .parent()
